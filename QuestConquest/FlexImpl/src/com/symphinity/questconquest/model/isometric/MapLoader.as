			...
			for(var i:int = 0; i < _grid.length; i++)
			{
				for(var j:int = 0; j < _grid[i].length; j++)
				{
					...
					switch(cell.type)
					{
						...
						gfx = new graphicClass() as DisplayObject;
						tile = new GraphicTile(size, gfx as Bitmap, parseInt(cell.xoffset), parseInt(cell.yoffset));
						break;
						
						case  "InteractiveGraphicsTile" :
						var interactiveGraphicClass:Class = getDefinitionByName(cell.graphicClass) as Class;
						gfx = new graphicClass() as DisplayObject;
						tile = new InteractiveGraphicsTile(size, gfx as Bitmap, parseInt(cell.xoffset), parseInt(cell.yoffset));
						break;
					...
					//trace(" In mapped world, tile ", i, j, "added at z-index ", world.getChildIndex(tile) );
				}
			}
			return world;
		}
